...
day11 = show . length . filter (=='#') . snd . head . dropWhile (uncurry (/=)) . uncurry zip . (id &&& tail) . iterate baz
  where
  rules ('L', 0)          = '#'
  rules ('#', n) | n >= 4 = 'L'
  rules (x  , _)          = x
...
day11b = show . sum . map (bar . snd) . take 1 . dropWhile (uncurry (/=)) . uncurry zip . (id &&& tail) . iterate baz
  where
  rules ('L', 0)          = '#'
  rules ('#', n) | n >= 5 = 'L'
  rules (x  , _)          = x
